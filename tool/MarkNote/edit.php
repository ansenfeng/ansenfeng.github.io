		...
		foreach ($theNotebooks as $value) {
			...
			if(is_array($value)){
				...
				<div class="notelist-folder">
					<i class="fa fa-angle-down fa-lg i-notelist-folder-arrow cang" aria-hidden="true"></i>
...
<body>
	...
	<div id="header">
		<h1 id="header-title">记事本</h1>
		<input class="input-btn"	type="submit"	name="submit" onclick="saveNote()"	value="保存" />
		<div id="header-user">
			...
			<span id="header-user-emailandlogout"><?php echo getUserEmail($USERNAME); ?> | <a style="cursor: pointer;" onclick="$('#header-user-logoutform').submit();">注销</a></span>
...
<!-- 		<div id="toolbar">
	 	...
	 	<div id="sidebar">
	 		<div id="sidebar-status">状态: <span id="sidebar-status-icon">●</span> <span id="sidebar-status-text">page loding...</span></div>
			<div id="sidebar-notelist">记事本载入</div>
	 	</div>
	 	<div id="editor">
	...
	<div id="contextmenu-1" class="contextmenu">
		...
		<div class="contextmenu-item" onclick="noteContextClick('clone');">		<i class="fa fa-clone" aria-hidden="true"></i> 克隆</div>
